...
The theoretical foundation of this paper lies in the canonical principal–agent framework. Holmstrom (1979) establishes that under CARA utility and normally distributed noise, optimal contracts are linear in output and balance incentives against risk exposure. Holmstrom and Milgrom (1987) show that this linearity result is not merely a tractable simplification but remains optimal once a principal aggregates performance over repeated periods — the formal justification for restricting attention, as this paper does throughout Section 4.3, to the linear piece-rate contract $w=\alpha+\gamma y$. Holmstrom and Milgrom (1991) extend the underlying insight to multi-task environments, showing that high-noise tasks favor fixed wages because incentives distort effort allocation.
...
Building on the linear-contract benchmark established by Holmstrom (1979) and Holmstrom and Milgrom (1987, 1991) — which this paper adopts rather than re-derives — and relative to these three strands, the paper makes four contributions:
...
To ensure internal consistency and empirical plausibility, the remaining parameters are calibrated as follows. Two of these — $r$ and $\sigma^2$ — are drawn from the empirical ranges established in Sections 3.1–3.3; the rest ($h$, $\theta$, $C$, $k$, $a_0$, $\bar U$, $F$) are internal-consistency normalizations rather than values fit or estimated from data, chosen so that monetary quantities are expressed as multiples of a representative baseline monthly wage (see the units note following the Notation table for the full mapping):
...
Figure 1 below summarizes this sequence.
...
### Step 2: Individual Rationality (IR)
...
Both $\Pi_T(A)$ and $\tilde H(A)$ depend on $\theta$ and $C$ only through the product $\phi\equiv\theta C$:

$$
\Pi_T(A) = a_0h + a_0\phi A - \bar U - \tfrac12 ka_0^2, \qquad \tilde H = h+\phi A.
$$

Substituting $x\equiv\phi A$, the equilibrium condition $G(A^*)=0$ becomes an equation purely in $x$ (all other parameters fixed), so its solution $x^*$ is a constant independent of how $\phi$ is decomposed into $\theta$ and $C$. Since $x^*=\phi A^*=\theta C\cdot A^*$, we obtain the exact closed form
...
All-China Federation of Trade Unions, Ministry of Human Resources and Social Security of the PRC, China Enterprise Confederation/China Enterprise Directors Association, & All-China Federation of Industry and Commerce. (2026, March). *Guidelines on Platform Labor Rules and Algorithm Negotiation (Trial)* [平台劳动规则和算法协商指引(试行)]. *Note: this document formally establishes, as an institutional mechanism, algorithm-and-labor-rules negotiation practice the ACFTU had already advanced in 2025 — that year, 15 major platform companies concluded negotiation agreements under this practice, covering more than 20 million new-form-employment workers.*
